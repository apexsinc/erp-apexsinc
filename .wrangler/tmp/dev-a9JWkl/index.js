...
// .wrangler/tmp/bundle-RnpNHn/strip-cf-connecting-ip-header.js
...
app.get("/", (c) => {
  return c.json({
    name: "Apexs ERP - Cloudflare Edge Business Management System",
    version: "1.0.0",
    runtime: "Cloudflare Workers (D1 + Drizzle ORM + Hono.js)",
    financialPrecision: "Integer Cents ($1.00 = 100)",
    modules: {
  ...
  async (c) => {
    ...
    if (totalDebit !== totalCredit) {
      return c.json(
        {
          success: false,
          error: `Double-entry violation: Total debits (${totalDebit}) must equal total credits (${totalCredit})`
        },
        400
      );
    }
    ...
    for (const entry of body.entries) {
      ...
      if (!acc)
        return c.json({ success: false, error: `Account with code ${entry.accountCode} not found` }, 404);
      entryInserts.push(
        db.insert(journalEntries).values({
          id: crypto.randomUUID(),
          voucherType: "JOURNAL",
...
app.post("/api/payroll/runs/:id/finalize", async (c) => {
  ...
  const run = await db.query.payrollRuns.findFirst({ where: eq(payrollRuns.id, runId) });
  if (!run)
    return c.json({ success: false, error: "Payroll Run not found" }, 404);
...
// .wrangler/tmp/bundle-RnpNHn/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-RnpNHn/middleware-loader.entry.ts
